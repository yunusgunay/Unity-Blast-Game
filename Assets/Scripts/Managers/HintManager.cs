        ...
        for (int y = 0; y < gameBoard.boardRows; ++y) {
            for (int x = 0; x < gameBoard.boardCols; ++x) {
                ...
                while (queue.Count > 0) {
                    ...
                    var matchedCells = GroupChecker.Instance.FindMatches(currCell, currCell.item.GetMatchType());
                    var matchedCubeCount = GroupChecker.Instance.CountGroupCubes(matchedCells);
